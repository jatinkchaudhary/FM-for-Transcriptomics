...
# Force line-buffered output only when this file is the training entrypoint.
# Benchmark adapters import ExpressionPerformer from this module; replacing the
# host process's stdout/stderr during import can close Jupyter or benchmark log
# descriptors when multiple checkpoints are loaded sequentially.
...
CONFIG = {
    ...
    'gene_prior_path': None,
    'gene_prior_init': False,
    'gene_prior_weight': 0.0,
    'context_prior_weight': 0.0,
    'sample_contrastive_weight': 0.0,
    'contrastive_temperature': 0.1,
    'accumulate_context_embeddings': False,
    'benchmark_context_weight': 0.0,
    'mask_ratio': 0.15,
    'mask_token': -10,
    'learning_rate': 1e-4,
    'weight_decay': 0,
    ...
    'resume_path': None,
    'resume_run_id': None,
    # Save latest.pt during long epochs. 0 disables intra-epoch checkpointing.
...
def _env_optional_int(value: str):
...
class ExpressionPerformer(nn.Module):
    ...
    def encode_hidden(self, x, species_ids=None):
        """
        Args:
            x: [batch, num_genes] expression values
        Returns:
            [batch, num_genes, hidden_dim] contextual gene states
        """
        B, G = x.shape
        device = x.device
    ...
    def forward(self, x, species_ids=None, return_hidden=False):
        h = self.encode_hidden(x, species_ids)
...
class StreamingParquetMLMDataset(Dataset):
    ...
    def __init__(self, batch_dir, sample_indices, normalization='tpm', mask_ratio=0.15,
                 ...
                 ddp_file_split=True, gene_columns=None, lengths_by_species=None):
        self.batch_dir = Path(batch_dir)
        self.batch_files = sorted(self.batch_dir.glob("*.parquet"))
        self.rank = int(rank)
        self.world_size = int(world_size)
        self.normalization = normalization
        self.lengths_by_species = lengths_by_species or {}
        self.mask_ratio = mask_ratio
        self.mask_token = mask_token
        ...
        available_gene_columns = [
            c for c in first_schema_cols
            if c not in ('geo_accession', '__index_level_0__')
        ]
        if gene_columns is None:
            self._gene_columns = available_gene_columns
        else:
            missing = sorted(set(gene_columns) - set(available_gene_columns))
            if missing:
                raise ValueError(
                    f"{len(missing)} requested genes are absent from parquet schema; "
                    f"first missing={missing[:5]}"
                )
            self._gene_columns = list(gene_columns)
    ...
    def collate_batch(self, batch_record_indices):
        ...
        elif self.normalization == 'counts_to_log1p_tpm':
            lengths = np.empty_like(x_true, dtype=np.float32)
            for species_id in np.unique(species_ids):
                species_lengths = self.lengths_by_species.get(int(species_id))
                if species_lengths is None:
                    raise ValueError(f"missing gene lengths for species id {species_id}")
                lengths[species_ids == species_id] = species_lengths
            rates = np.maximum(x_true, 0.0) / np.maximum(lengths / 1000.0, 1e-6)
            denom = rates.sum(axis=1, keepdims=True)
            tpm = rates / np.maximum(denom, 1e-12) * 1e6
            x_true = np.log1p(tpm).astype(np.float32, copy=False)
        elif self.normalization not in ('tpm', 'already_log1p_tpm', 'none'):
...
class ResumableDistributedSampler(DistributedSampler):
    """DistributedSampler with a deterministic sample offset for mid-epoch resume.

    The offset is applied before DataLoader workers fetch samples, avoiding an
    expensive replay of already-completed streaming batches.
    """
    ...
    def set_start_batch(self, batch_idx, batch_size):
        self.start_index = max(0, int(batch_idx) * int(batch_size))

    def clear_start_batch(self):
        self.start_index = 0
    ...
    def __iter__(self):
        ...
        if self.start_index <= 0:
            return base_iter
        return itertools.islice(base_iter, self.start_index, None)
...
def get_sample_indices(
    ...
    def _species_from_batch_file(batch_file):
        name = batch_file.name.lower()
        if name.startswith('human_') or name.startswith('human-'):
            return 'human'
        if name.startswith('mouse_') or name.startswith('mouse-'):
            return 'mouse'
        return None
    ...
    for sp in preferred_species_order:
        if any(v == sp for v in sample_to_species.values()) or any(
            _species_from_batch_file(f) == sp for f in batch_files
        ):
    ...
    if manifest_path.exists():
        ...
        for batch_idx, batch_file in enumerate(batch_files):
            file_species = _species_from_batch_file(batch_file)
            ...
            for sample_idx, sample_id in enumerate(sample_ids):
                species = file_species or _norm_species(sample_to_species.get(sample_id, "unknown"))
    ...
    if train_subset is None:
        if val_subset is not None:
            train_count = max(0, len(shuffled) - min(int(val_subset), len(shuffled)))
        else:
            # Backward-compatible default when neither split size is explicit.
...
def masked_reconstruction_mse(pred, target, mask_indices):
    """Vectorized MSE over the selected masked genes."""
    selected_pred = pred.gather(1, mask_indices)
    selected_target = target.gather(1, mask_indices)
    return F.mse_loss(selected_pred, selected_target)


def aligned_cosine_loss(values, targets, covered):
    if values is None or targets is None or covered is None or not bool(covered.any()):
        device = values.device if values is not None else targets.device
        return torch.zeros((), dtype=torch.float32, device=device)
    values = F.normalize(values[covered].float(), dim=-1)
    targets = F.normalize(targets[covered].float(), dim=-1)
    return 1.0 - (values * targets).sum(dim=-1).mean()


def contextual_prior_loss(hidden, species_ids, targets, covered):
    losses = []
    for species_id in torch.unique(species_ids):
        species_hidden = hidden[species_ids == species_id]
        if species_hidden.numel() == 0:
            continue
        losses.append(
            aligned_cosine_loss(species_hidden.mean(dim=0).float(), targets, covered)
        )
    if not losses:
...
def sample_contrastive_loss(hidden, x_masked, gene_embedding, mask_token, temperature):
    """Two gene-subset views from one forward pass, optimized with symmetric InfoNCE."""
    batch_size, num_genes, _ = hidden.shape
    if batch_size < 2:
        return torch.zeros((), dtype=torch.float32, device=hidden.device)
    context = hidden - gene_embedding.to(dtype=hidden.dtype).unsqueeze(0)
    observed = (x_masked != mask_token).to(dtype=hidden.dtype)
    split = (torch.rand(num_genes, device=hidden.device) >= 0.5).to(dtype=hidden.dtype)
    weight_a = observed * split.unsqueeze(0)
    weight_b = observed * (1.0 - split).unsqueeze(0)
    view_a = (context * weight_a.unsqueeze(-1)).sum(dim=1)
    view_b = (context * weight_b.unsqueeze(-1)).sum(dim=1)
    view_a = view_a / weight_a.sum(dim=1, keepdim=True).clamp(min=1.0)
    view_b = view_b / weight_b.sum(dim=1, keepdim=True).clamp(min=1.0)
    view_a = F.normalize(view_a.float(), dim=-1)
    view_b = F.normalize(view_b.float(), dim=-1)
    logits = view_a @ view_b.transpose(0, 1)
...
def build_benchmark_gene_embedding(model, contextual_mean, context_weight=0.0):
    static = F.normalize(model.gene_embedding.weight.detach().float(), dim=-1)
    contextual = F.normalize(contextual_mean.detach().float(), dim=-1)
    weight = max(0.0, float(context_weight))
    return F.normalize(static + weight * contextual, dim=-1)
...
def main():
    ...
    if is_main:
        ...
        env_overrides = {
            'model_name': (os.environ.get('TXN_MODEL_NAME') or os.environ.get('BRIDGE_MODEL_NAME'), str),
            'run_id': (os.environ.get('TXN_RUN_ID') or os.environ.get('BRIDGE_RUN_ID'), str),
            'data_dir': (os.environ.get('TXN_DATA_DIR') or os.environ.get('BRIDGE_DATA_DIR'), str),
            'checkpoint_dir': (os.environ.get('TXN_CHECKPOINT_DIR') or os.environ.get('BRIDGE_CHECKPOINT_DIR'), str),
            'hidden_dim': (os.environ.get('BRIDGE_HIDDEN_DIM'), int),
            'num_layers': (os.environ.get('BRIDGE_NUM_LAYERS'), int),
            'num_heads': (os.environ.get('BRIDGE_NUM_HEADS'), int),
            'ffn_dim': (os.environ.get('BRIDGE_FFN_DIM'), int),
            'batch_size': (os.environ.get('BRIDGE_BATCH_SIZE'), int),
            'learning_rate': (os.environ.get('BRIDGE_LR'), float),
            'weight_decay': (os.environ.get('BRIDGE_WEIGHT_DECAY'), float),
            'mask_ratio': (os.environ.get('BRIDGE_MASK_RATIO'), float),
            'ree_base': (os.environ.get('BRIDGE_REE_BASE'), float),
            'data_mode': (os.environ.get('BRIDGE_DATA_MODE'), str),
            'stream_ddp_file_split': (os.environ.get('BRIDGE_STREAM_DDP_FILE_SPLIT'), _env_bool),
            'stream_cache_size': (os.environ.get('BRIDGE_STREAM_CACHE_SIZE'), int),
            'num_workers': (os.environ.get('BRIDGE_NUM_WORKERS'), int),
            'prefetch_factor': (os.environ.get('BRIDGE_PREFETCH_FACTOR'), int),
            'persistent_workers': (os.environ.get('BRIDGE_PERSISTENT_WORKERS'), _env_bool),
            'train_subset': (os.environ.get('BRIDGE_TRAIN_SUBSET'), _env_optional_int),
            'val_subset': (os.environ.get('BRIDGE_VAL_SUBSET'), _env_optional_int),
            'balanced_sampling': (os.environ.get('BRIDGE_BALANCED_SAMPLING'), _env_bool),
            'balanced_shards_only': (os.environ.get('BRIDGE_BALANCED_SHARDS_ONLY'), _env_bool),
            'balanced_shard_tolerance': (os.environ.get('BRIDGE_BALANCED_SHARD_TOLERANCE'), int),
            'max_balanced_shards': (os.environ.get('BRIDGE_MAX_BALANCED_SHARDS'), _env_optional_int),
            'include_species_embedding': (os.environ.get('BRIDGE_INCLUDE_SPECIES_EMBEDDING'), _env_bool),
            'normalization': (os.environ.get('BRIDGE_NORMALIZATION'), str),
            'gene_prior_path': (os.environ.get('BRIDGE_GENE_PRIOR_PATH'), str),
            'gene_prior_init': (os.environ.get('BRIDGE_GENE_PRIOR_INIT'), _env_bool),
            'gene_prior_weight': (os.environ.get('BRIDGE_GENE_PRIOR_WEIGHT'), float),
            'context_prior_weight': (os.environ.get('BRIDGE_CONTEXT_PRIOR_WEIGHT'), float),
            'sample_contrastive_weight': (os.environ.get('BRIDGE_SAMPLE_CONTRASTIVE_WEIGHT'), float),
            'contrastive_temperature': (os.environ.get('BRIDGE_CONTRASTIVE_TEMPERATURE'), float),
            'accumulate_context_embeddings': (
                os.environ.get('BRIDGE_ACCUMULATE_CONTEXT_EMBEDDINGS'), _env_bool
            ),
            'benchmark_context_weight': (
                os.environ.get('BRIDGE_BENCHMARK_CONTEXT_WEIGHT'), float
            ),
            'min_lr_ratio': (os.environ.get('BRIDGE_MIN_LR_RATIO'), float),
            'early_stopping': (os.environ.get('BRIDGE_EARLY_STOPPING'), _env_bool),
            'patience': (os.environ.get('BRIDGE_PATIENCE'), int),
            'progress_log_interval_sec': (os.environ.get('BRIDGE_PROGRESS_LOG_INTERVAL_SEC'), float),
            'timing_profile': (os.environ.get('BRIDGE_TIMING_PROFILE'), _env_bool),
            'checkpoint_interval_batches': (os.environ.get('BRIDGE_CHECKPOINT_INTERVAL_BATCHES'), int),
        }
        ...
        for key, (value, caster) in env_overrides.items():
            if value is None:
                continue
    ...
    if is_main and HAS_WANDB:
        ...
        try:
            wandb.init(
                ...
                name=os.environ.get("WANDB_NAME", CONFIG.get("model_name")),
                config=CONFIG,
    ...
    if not batch_dir.exists():
        batch_dir = data_dir
    gene_columns = None
    length_arrays_by_name = {}
    normalization_provenance = None
    if CONFIG.get('normalization') == 'counts_to_log1p_tpm':
        canonical_path = data_dir / "canonical_genes.csv"
        lengths_path = data_dir / "gene_lengths.npz"
        manifest_path = data_dir / "normalization_manifest.json"
        if not canonical_path.exists() or not lengths_path.exists() or not manifest_path.exists():
            raise FileNotFoundError(
                "counts_to_log1p_tpm requires canonical_genes.csv, "
                "gene_lengths.npz, and normalization_manifest.json in data_dir"
            )
        canonical_df = pd.read_csv(canonical_path)
        gene_columns = canonical_df["gene_symbol"].astype(str).tolist()
        with np.load(lengths_path, allow_pickle=False) as lengths_npz:
            length_genes = lengths_npz["genes"].astype(str).tolist()
            if length_genes != gene_columns:
                raise ValueError("gene_lengths.npz order does not match canonical_genes.csv")
            length_arrays_by_name = {
                "human": lengths_npz["human"].astype(np.float32),
                "mouse": lengths_npz["mouse"].astype(np.float32),
            }
        normalization_provenance = json.loads(manifest_path.read_text())
    ...
    lengths_by_species = {
        int(species_to_id[name]): values
        for name, values in length_arrays_by_name.items()
    ...
    if data_mode == 'streaming':
        ...
        train_ds = StreamingParquetMLMDataset(
            ...
            lengths_by_species=lengths_by_species,
        )
        ...
        num_genes = len(gene_columns) if gene_columns is not None else get_num_genes_from_batches(batch_dir)
    else:
        if is_main:
    ...
    gene_prior = None
    gene_prior_covered = None
    gene_prior_path = CONFIG.get('gene_prior_path')
    if gene_prior_path:
        prior_path = Path(gene_prior_path)
        if not prior_path.exists():
            raise FileNotFoundError(f"gene prior not found: {prior_path}")
        with np.load(prior_path, allow_pickle=False) as prior_npz:
            prior_genes = prior_npz["genes"].astype(str).tolist()
            prior_embeddings = prior_npz["embeddings"].astype(np.float32)
            prior_covered = prior_npz["covered"].astype(bool)
        expected_genes = gene_columns
        if expected_genes is None:
            canonical_path = data_dir / "canonical_genes.csv"
            if canonical_path.exists():
                expected_genes = pd.read_csv(canonical_path)["gene_symbol"].astype(str).tolist()
        if expected_genes is not None and prior_genes != expected_genes:
            raise ValueError("gene prior order does not match the training canonical gene order")
        if prior_embeddings.shape != (num_genes, int(CONFIG['hidden_dim'])):
            raise ValueError(
                f"gene prior shape={prior_embeddings.shape}, "
                f"expected={(num_genes, int(CONFIG['hidden_dim']))}"
            )
        gene_prior = torch.from_numpy(prior_embeddings).to(device=device)
        gene_prior_covered = torch.from_numpy(prior_covered).to(device=device)
        if bool(CONFIG.get('gene_prior_init', False)):
            init_scale = math.sqrt(float(CONFIG['hidden_dim']))
            with torch.no_grad():
                model.gene_embedding.weight[gene_prior_covered] = (
                    gene_prior[gene_prior_covered] * init_scale
                )
        if is_main:
            print(
                f"[PRIOR] Loaded {int(gene_prior_covered.sum().item()):,}/{num_genes:,} "
                f"aligned genes from {prior_path}",
                flush=True,
            )
    ...
    if is_main:
        ...
        print(
            f"  ✓ LR schedule: linear warmup {warmup_ratio:.1%} "
            f"({warmup_steps}/{total_train_steps} steps) + cosine decay "
            f"(floor={min_lr_ratio:.1%})"
        )
    ...
    resume_running_total_loss = 0.0
    resume_running_gene_prior_loss = 0.0
    resume_running_context_prior_loss = 0.0
    resume_running_sample_contrastive_loss = 0.0
    resume_num_batches = 0
    resume_context_sum = None
    ...
    if latest_ckpt is not None:
        ...
        checkpoint_kind = str(ckpt.get('checkpoint_kind', 'epoch'))
        start_epoch = int(ckpt['epoch'])
        if checkpoint_kind == 'batch':
            start_batch = int(ckpt.get('batch_idx', -1)) + 1
            resume_running_loss = float(ckpt.get('running_loss', 0.0))
            resume_running_total_loss = float(
                ckpt.get('running_total_loss', resume_running_loss)
            )
            resume_running_gene_prior_loss = float(
                ckpt.get('running_gene_prior_loss', 0.0)
            )
            resume_running_context_prior_loss = float(
                ckpt.get('running_context_prior_loss', 0.0)
            )
            resume_running_sample_contrastive_loss = float(
                ckpt.get('running_sample_contrastive_loss', 0.0)
            )
            resume_num_batches = int(ckpt.get('num_batches', 0))
            resume_context_sum = ckpt.get('context_sum')
            resume_context_count = int(ckpt.get('context_count', 0))
        if is_main:
            if start_batch > 0:
                print(
                    f"[RESUME] Restarting from epoch {start_epoch}, batch {start_batch}",
                    flush=True,
                )
            else:
    ...
    patience_counter = 0
    train_losses, train_total_losses, val_losses = [], [], []
    gene_prior_losses, context_prior_losses, sample_contrastive_losses = [], [], []
    ...
    canonical_gene_source = next(
        (p for p in [
            data_dir / "canonical_genes.csv",
            data_dir.parent / "canonical_genes.csv",
            data_dir / "genes.csv",
        ] if p.exists()),
        None,
    )
    if is_main and canonical_gene_source is not None:
        shutil.copy2(canonical_gene_source, ckpt_dir / "canonical_genes.csv")
        print(f"[DATA] Copied canonical genes -> {ckpt_dir / 'canonical_genes.csv'}", flush=True)
    ...
    run_metadata = {
        'model_name': CONFIG.get('model_name', 'BRIDGE'),
        'run_id': run_id,
        ...
        'representation_objectives': {
            'gene_prior_path': str(gene_prior_path) if gene_prior_path else None,
            'gene_prior_init': bool(CONFIG.get('gene_prior_init', False)),
            'gene_prior_weight': float(CONFIG.get('gene_prior_weight', 0.0) or 0.0),
            'context_prior_weight': float(CONFIG.get('context_prior_weight', 0.0) or 0.0),
            'sample_contrastive_weight': float(
                CONFIG.get('sample_contrastive_weight', 0.0) or 0.0
            ),
            'contrastive_temperature': float(
                CONFIG.get('contrastive_temperature', 0.1)
            ),
            'accumulate_context_embeddings': bool(
                CONFIG.get('accumulate_context_embeddings', False)
            ),
            'contextual_embedding_source': (
                'frozen_end_of_epoch_validation_mean'
                if CONFIG.get('accumulate_context_embeddings', False)
                else None
            ),
            'training_contextual_embedding_source': (
                'online_full_training_epoch_mean'
                if CONFIG.get('accumulate_context_embeddings', False)
                else None
            ),
            'benchmark_context_weight': float(
                CONFIG.get('benchmark_context_weight', 0.0)
            ),
        },
        'sweep_parameters': {
        ...
        'dataset': {
            ...
            'canonical_genes': str(canonical_gene_source) if canonical_gene_source is not None else None,
    ...
    for epoch in range(start_epoch, CONFIG['epochs']):
        ...
        resume_sampler_offset_active = bool(
            resume_this_epoch
            and not stream_use_file_shard
            and hasattr(train_sampler, 'set_start_batch')
        )
        if not stream_use_file_shard and hasattr(train_sampler, 'clear_start_batch'):
            train_sampler.clear_start_batch()
            if resume_sampler_offset_active:
                train_sampler.set_start_batch(
                    start_batch, int(CONFIG['batch_size'])
                )
        running_recon_loss = resume_running_loss if resume_this_epoch else 0.0
        running_total_loss = resume_running_total_loss if resume_this_epoch else 0.0
        running_gene_prior_loss = (
            resume_running_gene_prior_loss if resume_this_epoch else 0.0
        )
        running_context_prior_loss = (
            resume_running_context_prior_loss if resume_this_epoch else 0.0
        )
        running_sample_contrastive_loss = (
            resume_running_sample_contrastive_loss if resume_this_epoch else 0.0
        )
        num_batches = resume_num_batches if resume_this_epoch else 0
        ...
        checkpoint_interval_batches = max(0, int(CONFIG.get('checkpoint_interval_batches', 0) or 0))
        gene_prior_weight = float(CONFIG.get('gene_prior_weight', 0.0) or 0.0)
        context_prior_weight = float(CONFIG.get('context_prior_weight', 0.0) or 0.0)
        sample_contrastive_weight = float(
            CONFIG.get('sample_contrastive_weight', 0.0) or 0.0
        ...
        need_hidden = bool(
            gene_prior_weight > 0.0
            or context_prior_weight > 0.0
            or sample_contrastive_weight > 0.0
            or CONFIG.get('accumulate_context_embeddings', False)
        )
        context_sum = (
            (
                resume_context_sum.to(device=device, dtype=torch.float32)
                if resume_this_epoch and resume_context_sum is not None
                else torch.zeros(
                    (num_genes, int(CONFIG['hidden_dim'])),
                    dtype=torch.float32,
                    device=device,
                )
            )
        ...
        context_count = resume_context_count if resume_this_epoch else 0
        ...
        if is_main and resume_this_epoch:
            if resume_sampler_offset_active:
                print(
                    f"[RESUME] Sampler offset applied for {start_batch:,} "
                    f"completed batches in epoch {epoch + 1}.",
                    flush=True,
                )
            else:
                print(
                    f"[RESUME] Skipping {start_batch:,} completed batches "
                    f"in epoch {epoch + 1}.",
                    flush=True,
                )

        batch_idx_offset = start_batch if resume_sampler_offset_active else 0
        for loader_batch_idx, (x_masked, x_true, mask_idx, species_id) in enumerate(train_loader):
            batch_idx = loader_batch_idx + batch_idx_offset
            if resume_this_epoch and not resume_sampler_offset_active and batch_idx < start_batch:
                continue
            ...
            with torch.amp.autocast(device_type='cuda', dtype=amp_dtype_torch, enabled=amp_enabled):
                if need_hidden:
                    pred, hidden = model(x_masked, species_id, return_hidden=True)
                else:
            ...
            t0 = time.perf_counter()
            with torch.amp.autocast(device_type='cuda', dtype=amp_dtype_torch, enabled=amp_enabled):
                recon_loss = masked_reconstruction_mse(pred, x_true, mask_idx)
            gene_identity_loss = torch.zeros((), dtype=torch.float32, device=device)
            context_alignment_loss = torch.zeros((), dtype=torch.float32, device=device)
            contrastive_loss = torch.zeros((), dtype=torch.float32, device=device)
            if gene_prior is not None and gene_prior_weight > 0.0:
                gene_identity_loss = aligned_cosine_loss(
                    model.module.gene_embedding.weight,
                    gene_prior,
                    gene_prior_covered,
                )
            if gene_prior is not None and context_prior_weight > 0.0:
                context_alignment_loss = contextual_prior_loss(
                    hidden,
                    species_id,
                    gene_prior,
                    gene_prior_covered,
                )
            if sample_contrastive_weight > 0.0:
                contrastive_loss = sample_contrastive_loss(
                    hidden,
                    x_masked,
                    model.module.gene_embedding.weight,
                    CONFIG['mask_token'],
            ...
            loss = (
                recon_loss.float()
                + gene_prior_weight * gene_identity_loss
                + context_prior_weight * context_alignment_loss
                + sample_contrastive_weight * contrastive_loss
            )
            if context_sum is not None:
                context_sum.add_(hidden.detach().float().sum(dim=0))
            ...
            recon_loss_item = float(recon_loss.item())
            gene_prior_loss_item = float(gene_identity_loss.item())
            context_prior_loss_item = float(context_alignment_loss.item())
            contrastive_loss_item = float(contrastive_loss.item())
            running_recon_loss += recon_loss_item
            running_total_loss += loss_item
            running_gene_prior_loss += gene_prior_loss_item
            running_context_prior_loss += context_prior_loss_item
            running_sample_contrastive_loss += contrastive_loss_item
            num_batches += 1
                ...
                if world_size > 1:
                    train_metric_buf = torch.tensor(
                        [
                            ...
                            float(
                                num_batches
                                - (resume_num_batches if resume_this_epoch else 0)
                            ),
                            train_step_s,
                        ],
                        device=device,
                    ...
                    global_total_loss = (train_metric_buf[0] / world_size).item()
                    global_recon_loss = (train_metric_buf[1] / world_size).item()
                    global_avg_total = (
                        train_metric_buf[2] / train_metric_buf[4].clamp(min=1.0)
                    ).item()
                    global_avg_recon = (
                        train_metric_buf[3] / train_metric_buf[4].clamp(min=1.0)
                    ).item()
                    global_avg_step = (
                        train_metric_buf[6] / train_metric_buf[5].clamp(min=1.0)
                    ).item()
                else:
                    global_total_loss = loss_item
                    global_recon_loss = recon_loss_item
                    global_avg_total = running_total_loss / max(1, num_batches)
                    global_avg_recon = running_recon_loss / max(1, num_batches)
                    executed_batches = (
                        num_batches
                        - (resume_num_batches if resume_this_epoch else 0)
                    )
                    global_avg_step = train_step_s / max(1, executed_batches)

                if is_main:
                    print(f"  Epoch {epoch+1}/{CONFIG['epochs']} | "
                          f"Batch {batch_idx+1}/{len(train_loader)} | "
                          f"Recon: {global_recon_loss:.6f} | Total: {global_total_loss:.6f} | "
                          f"AvgRecon: {global_avg_recon:.6f} | AvgTotal: {global_avg_total:.6f} | "
            ...
            if (
                ...
                and (batch_idx + 1) % checkpoint_interval_batches == 0
            ):
                batch_checkpoint_payload = {
                    ...
                    'running_loss': running_recon_loss,
                    'running_total_loss': running_total_loss,
                    'running_gene_prior_loss': running_gene_prior_loss,
                    'running_context_prior_loss': running_context_prior_loss,
                    'running_sample_contrastive_loss': running_sample_contrastive_loss,
                    'num_batches': num_batches,
                    'context_sum': (
                        context_sum.detach().to("cpu", dtype=torch.float32)
                        if context_sum is not None else None
                    ),
                    'context_count': int(context_count),
                    'train_loss': running_recon_loss / max(1, num_batches),
                ...
                torch.save(batch_checkpoint_payload, ckpt_dir / "latest.pt")
                torch.save(batch_checkpoint_payload, ckpt_dir / "latest_batch.pt")
                print(
                    f"  [CHECKPOINT] Saved latest.pt at epoch {epoch + 1}, "
                    f"batch {batch_idx + 1}/{len(train_loader)}",
                    flush=True,
                )

        epoch_train_loss = running_recon_loss / max(1, num_batches)
        epoch_train_total_loss = running_total_loss / max(1, num_batches)
        epoch_gene_prior_loss = running_gene_prior_loss / max(1, num_batches)
        epoch_context_prior_loss = running_context_prior_loss / max(1, num_batches)
        epoch_sample_contrastive_loss = (
            running_sample_contrastive_loss / max(1, num_batches)
        )
        ...
        with torch.no_grad():
            for x_masked, x_true, mask_idx, species_id in val_loader:
                ...
                t0 = time.perf_counter()
                with torch.amp.autocast(device_type='cuda', dtype=amp_dtype_torch, enabled=amp_enabled):
                    if val_context_sum is not None:
                        pred, val_hidden = model(
                            x_masked, species_id, return_hidden=True
                        )
                    else:
                        pred = model(x_masked, species_id)
                        val_hidden = None
                ...
                t0 = time.perf_counter()
                with torch.amp.autocast(device_type='cuda', dtype=amp_dtype_torch, enabled=amp_enabled):
                    val_batch_loss = masked_reconstruction_mse(pred, x_true, mask_idx)
                if val_context_sum is not None:
                    val_context_sum.add_(val_hidden.detach().float().sum(dim=0))
                    val_context_count += int(val_hidden.shape[0])
        ...
        training_contextual_gene_mean = None
        contextual_gene_mean = None
        benchmark_gene_embedding = None
        if context_sum is not None:
            context_count_t = torch.tensor(
                float(context_count), dtype=torch.float64, device=device
            )
            dist.all_reduce(context_sum, op=dist.ReduceOp.SUM)
            dist.all_reduce(context_count_t, op=dist.ReduceOp.SUM)
            training_contextual_gene_mean = (
                context_sum / context_count_t.clamp(min=1.0)
            )
        if val_context_sum is not None:
            val_context_count_t = torch.tensor(
                float(val_context_count), dtype=torch.float64, device=device
            )
            dist.all_reduce(val_context_sum, op=dist.ReduceOp.SUM)
            dist.all_reduce(val_context_count_t, op=dist.ReduceOp.SUM)
            contextual_gene_mean = (
                val_context_sum / val_context_count_t.clamp(min=1.0)
            )
        elif training_contextual_gene_mean is not None:
            contextual_gene_mean = training_contextual_gene_mean
            val_context_count_t = context_count_t
        if contextual_gene_mean is not None:
            benchmark_gene_embedding = build_benchmark_gene_embedding(
                model.module,
                contextual_gene_mean,
                CONFIG.get('benchmark_context_weight', 0.0),
            )
        ...
        gene_prior_losses.append(epoch_gene_prior_loss)
        context_prior_losses.append(epoch_context_prior_loss)
        sample_contrastive_losses.append(epoch_sample_contrastive_loss)
        ...
        if is_main and wandb_active:
            wandb.log({
                ...
                'gene_prior_loss': epoch_gene_prior_loss,
                'context_prior_loss': epoch_context_prior_loss,
                'sample_contrastive_loss': epoch_sample_contrastive_loss,
        ...
        if is_main:
            ...
            print(f"\n  ╔════════════════════════════════════════════╗")
            print(f"  ║ Epoch {epoch+1}/{CONFIG['epochs']}")
            print(f"  ║ Train Recon Loss: {epoch_train_loss:.6f}")
            print(f"  ║ Train Total Loss: {epoch_train_total_loss:.6f}")
            print(f"  ║ Val Recon Loss:   {epoch_val_loss:.6f}")
            if gene_prior is not None:
                print(
                    f"  ║ Prior Losses: gene={epoch_gene_prior_loss:.6f}, "
                    f"context={epoch_context_prior_loss:.6f}"
                )
            if sample_contrastive_weight > 0.0:
                print(
                    f"  ║ Sample Contrastive Loss: "
                    f"{epoch_sample_contrastive_loss:.6f}"
                )
            ...
            if timing_profile:
                ...
                denom = max(1, executed_batches)
                global_train_samples = (
                    executed_batches * int(CONFIG['batch_size']) * world_size
                )
            ...
            checkpoint_payload = {
                ...
                'context_prior_loss': epoch_context_prior_loss,
                'sample_contrastive_loss': epoch_sample_contrastive_loss,
                'contextual_gene_embedding': (
                    contextual_gene_mean.detach().to("cpu", dtype=torch.float16)
                    if contextual_gene_mean is not None else None
                ),
                'training_contextual_gene_embedding': (
                    training_contextual_gene_mean.detach().to(
                        "cpu", dtype=torch.float16
                    )
                    if training_contextual_gene_mean is not None else None
                ),
                'benchmark_gene_embedding': (
                    benchmark_gene_embedding.detach().to("cpu", dtype=torch.float16)
                    if benchmark_gene_embedding is not None else None
                ),
                'contextual_gene_embedding_samples': (
                    int(val_context_count_t.item())
                    if contextual_gene_mean is not None else 0
                ),
                'training_contextual_gene_embedding_samples': (
                    int(context_count_t.item()) if context_sum is not None else 0
                ),
                'config': dict(CONFIG),
                'run_metadata': run_metadata,
                'total_params': total_params,
    ...
    if is_main:
        ...
        pd.DataFrame({
            ...
            'train_recon_loss': train_losses,
            'train_total_loss': train_total_losses,
            'val_recon_loss': val_losses,
            'gene_prior_loss': gene_prior_losses,
            'context_prior_loss': context_prior_losses,
            'sample_contrastive_loss': sample_contrastive_losses,
        }).to_csv(ckpt_dir / "loss_history.csv", index=False)

        # Loss plot
        if HAS_MATPLOTLIB:
            plt.figure(figsize=(10, 6))
            plt.plot(train_losses, marker='o', label='Train reconstruction', linewidth=2)
            plt.plot(train_total_losses, marker='.', label='Train total', linewidth=1)
            plt.plot(val_losses, marker='s', label='Validation reconstruction', linewidth=2)
            plt.xlabel("Epoch")
            plt.ylabel("MSE Loss")
